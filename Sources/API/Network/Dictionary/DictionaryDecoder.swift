...
extension _DictionaryDecoder {
    /// Helper to check if an NSNumber represents a boolean value
    private func isBooleanNumber(_ number: NSNumber) -> Bool {
        #if canImport(CoreFoundation)
        // Use toll-free bridging to compare CFBoolean with NSNumber
        if let cfTrue = kCFBooleanTrue {
            let nsTrue = unsafeBitCast(cfTrue, to: NSNumber.self)
            if number === nsTrue {
                return true
            }
        }
        if let cfFalse = kCFBooleanFalse {
            let nsFalse = unsafeBitCast(cfFalse, to: NSNumber.self)
            if number === nsFalse {
                return true
            }
        }
        ...
        // Check the objCType to determine if this is a boolean
        let objCType = number.objCType.pointee
        return objCType == Int8(UInt8(ascii: "c")) || objCType == Int8(UInt8(ascii: "B"))
    }
    ...
    fileprivate func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
        ...
        if let number = value as? NSNumber {
            ...
            // Check the actual objCType to determine if this is a boolean
            let objCType = number.objCType.pointee
            if objCType == Int8(UInt8(ascii: "c")) || objCType == Int8(UInt8(ascii: "B")) {
    ...
    fileprivate func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
        ...
        if let number = value as? NSNumber {
            // Check if it's a boolean number and reject it
            #if canImport(CoreFoundation)
            if let cfTrue = kCFBooleanTrue {
                let nsTrue = unsafeBitCast(cfTrue, to: NSNumber.self)
                if number === nsTrue {
    ...
    fileprivate func unbox(_ value: Any, as type: UUID.Type) throws -> UUID? {
        ...
        if cfType == CFUUIDGetTypeID() {
            ...
            if let cfString = CFUUIDCreateString(kCFAllocatorDefault, cfValue) {
                // CFString is toll-free bridged with NSString, cast using unsafeBitCast
                let string = unsafeBitCast(cfString, to: NSString.self) as String
                return UUID(uuidString: string)
            }
        }
        #endif
    ...
    /// Helper to check if a type is a UUID-related type
    private func isUUIDType<T>(_ type: T.Type) -> Bool {
        if type == UUID.self {
            return true
        }
        #if canImport(CoreFoundation)
